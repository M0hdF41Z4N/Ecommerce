...
import { cartSelector, clearCartAsync ,getCartDataAsync } from "../redux/reducers/cartReducer";
import { placeOrderAsync } from "../redux/reducers/orderReducer";
...
export function Cart(){
    ...
    const {cartItems,total,items}=cartState;
    
    // data of user from custom hook (authentication)
    const authState = useSelector(authSelector);
    const {userLoggedIn,isLoggedIn} = authState;
    ...
    function handlePurchase(){
        ...
        // dispatch purchase function
        dispatch(placeOrderAsync({cartItems,total}));
        // show notification
            ...
            <div className={secondStyles.mainContainer}>
                ...
                <div className={firstStyles.itemContainer}>
                    ...
                    {cartItems.length === 0 ?
                                    ...
                                    :cartItems.map((product,i) => <CartItem key={i}
                                                        product={product}/>)}
                </div>
            </div>
        }
